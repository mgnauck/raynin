...
share_data *find_shared(uint8_t share_id, share_data *shared, uint32_t *share_cnt)
{
  if(share_id == 0)
    // Not shared
    return NULL;
...
uint8_t export_bin(uint8_t **bin, size_t *bin_sz, escene *scenes, uint8_t scene_cnt)
{
      ...
      if(em->type == OT_MESH) {
        ...
        if(!sd || sd->vertices_ofs == 0) {
          ...
          for(uint16_t k=0; k<em->index_cnt; k++)
            p = ie_write(p, &em->vertices[em->indices[k]], sizeof(*em->vertices));
          logc("Wrote %i vertices starting at ofs: %i until ofs: %i for scene: %i and mesh: %i", em->index_cnt, em->vertices_ofs, p - *bin, j, i);
#endif
        } else
