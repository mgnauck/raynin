...
void      scene_init(scene *s, uint16_t mesh_cnt, uint16_t mtl_cnt, uint16_t cam_cnt, uint16_t inst_cnt);
...
mesh      *scene_acquire_mesh(scene *s); // Pair calls to acquire and attach, i.e. can not acquire twice in a row
